...
const cmd = {
  async check(iEnv) {
    ...
    return await she.start({ env: iEnv, shortEnv: {}, cmds: [] });
  },
  path(iEnv) {
  ...
  async se({ env, shortEnv, cmds }) {
    const sePath = require('selenium-server').path;
    const cmd = `java -jar ${sePath} ${cmds.join(' ')} ${util.envStringify(env)} ${util.shortEnvStringify(shortEnv)}`;
    print.log.cmd(cmd);
  ...
  async install({ cmds, env, shortEnv }) {
    const rootPath = path.join(__dirname, '../');
    const cmd = `npm i ${cmds.join(' ')} ${util.envStringify(env)} ${util.shortEnvStringify(shortEnv)}`;
    print.log.cmd(cmd);
    return await extOs.runCMD(cmd, rootPath);
  },
  help(iEnv) {
    const h = {
      usage: 'yyt',
