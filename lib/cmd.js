...
const cmd = {
  ...
  async doctor({ env }) {
    ...
    if (!env.silent) {
      ...
      let cdStr; = chalk.green.bold(chromeDriverVer);
      const reinstallVer = fn.disMatchChromeDriver(chromeVer, chromeDriverVer);
      if (reinstallVer) {
        cdStr = `${chalk.yellow(chromeDriverVer)} (require ${chal,yellow(reinstallVer)})`;
      } else {
      ...
      if (reinstallVer) {
        if (disMatchVer) {
          ...
          const cmd = `npm i chromedriver@${reinstallVer} --save`;
          print.log.cmd(cmd);
